...
from .serializers import RegisterUserSerializer, UserLoginSerializer, UserLogoutSerializer, TokenExchangeSerializer, GetAccountSerializer, GetTransactionsSerializer, TransactionUpdateSerializer
...
class Login(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        print("Pankaj")
        print(serializer_class.data['token'])
...
class TokenExchange(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        print(request.data.get('public_token'))
        if serializer_class.is_valid(raise_exception=True):
            return Response(serializer_class.data, status=HTTP_200_OK)
        return Response(serializer_class.errors, status=HTTP_400_BAD_REQUEST)
